...
export function CyberHeader() {
  const [statusText, setStatusText] = useState("SYSTEM_ONLINE")
  ...
  const navItems: NavItem[] = [
    { label: "DASHBOARD", href: "#dashboard", icon: <Activity className="w-4 h-4" />, active: true },
    { label: "AI_ANALYZER", href: "#analyzer", icon: <Terminal className="w-4 h-4" /> },
    { label: "CREATE_DEAL", href: "#create", icon: <Zap className="w-4 h-4" /> },
  ]
      ...
      <div className="border-b border-border/30 bg-card/50">
        <div className="container mx-auto px-4 py-1 flex items-center justify-between text-xs font-mono">
          <div className="flex items-center gap-4 text-muted-foreground">
            ...
            <span className="hidden sm:inline text-neon-cyan/70">NODE::MAINNET_V2.4</span>
          </div>
          <div className="flex items-center gap-4 text-muted-foreground">
        ...
        <div className="flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] font-mono text-muted-foreground tracking-widest">DARKNET PROTOCOL</span>
            </div>
          </Link>
            ...
            {navItems.map((item) => (
              <Link
                ...
                className={cn(
                  ...
                  item.active && "text-primary border-primary/30 bg-primary/5",
                )}
              >
                {item.icon}
                {item.label}
